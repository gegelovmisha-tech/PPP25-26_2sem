...
def test_update_item_patch():
    client.post("/sources/", json={"name": "Test Source"})
    client.post("/items/", json={"name": "Old Name", "price": 10, "source_id": 1})
    
    response = client.patch("/items/1", json={"name": "New Name"})
    assert response.status_code == 200
...
def test_filter_items_by_category():
    ...
    client.post("/items/", json={"name": "TV", "category": "Electronics", "source_id": 1})
