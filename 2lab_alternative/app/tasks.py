...
celery_app.conf.update(task_track_started=True, task_time_limit=30 * 60, result_expires=3600)
    ...
    try:
        ...
        for i, item in enumerate(items):
            ...
            if total > 0:
                ...
                self.update_state(state="RUNNING", meta={"progress": progress, "message": f"Processing {i+1}/{total}"})
            time.sleep(0.1)
        self.update_state(state="RUNNING", meta={"progress": 90, "message": "Calculating results..."})
        time.sleep(0.5)
        result = {"source_id": source_id, "total_items": total, "total_value": round(total_value, 2), "average_price": round(total_value / total, 2) if total > 0 else 0, "categories": categories, "status": "completed"}
...
def reimport_source(self, source_id: int):
    ...
    for i in range(1, 11):
        time.sleep(0.5)
        self.update_state(state="RUNNING", meta={"progress": i * 10, "message": f"Reimporting... step {i}/10"})
    return {"source_id": source_id, "status": "completed", "items_imported": random.randint(5, 50), "message": f"Successfully reimported source {source_id}"}
