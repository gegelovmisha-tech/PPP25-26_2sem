...
async def start_rebuild_stats(source_id: int = None):
    ...
    return TaskResponse(task_id=task.id, status="pending", message=f"Task started. Check /tasks/{task.id} for status")
...
async def start_reimport_source(source_id: int):
    ...
    return TaskResponse(task_id=task.id, status="pending", message=f"Reimport task started for source {source_id}")
...
async def get_task_status(task_id: str):
    ...
    return TaskStatus(task_id=task_id, status=status.lower(), progress=meta.get("progress"), message=meta.get("message") or meta.get("error"), result=result if task_result.successful() else None, error=error)
